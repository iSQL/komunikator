...
const Tile = ({ tile, onTap, onLongPress }: TileProps) => {
  ...
  return (
    <button
      className="flex flex-col items-center justify-center gap-1 rounded-xl p-2 shadow-md active:scale-95 transition-transform min-h-[80px] cursor-pointer border-2 border-white/30 select-none"
    ...
    >
      <div className="flex-1 flex items-center justify-center w-full">
        {isDataUrl ? (
          <img src={tile.symbolPath} alt="" className="w-12 h-12 rounded-lg object-contain" />
        ) : (
